...
module.exports = function makePublicShopRouter({ db, helpers, ordersEvents }) {
  ...
  async function loadPublicProductBlockRows(tenantId, productIds = []) {
  ...
  async function applyPublicProductBlocksToRows(rows, tenantId, storeId) {
    ...
    list.forEach((row) => {
      ...
      if (!blocksConfig.ingredients) {
        row.client_composition = null;
      }
      if (!blocksConfig.nutrition) {
        row.nutrition_kcal_100g = null;
        row.nutrition_per_100g = null;
        row.nutrition_per_portion = null;
        row.nutrition_incomplete = false;
      }
      if (Number(row.show_description_short) === 0) row.description_short = null;
      if (Number(row.show_description) === 0) row.description = null;
      if (Number(row.show_client_composition) === 0) row.client_composition = null;
      if (Number(row.show_tech_process) === 0) row.tech_process = null;
    });
  }

  function readPublicNutritionNumber(value) {
    if (value === null || value === undefined || value === '') return null;
    const num = Number(String(value).replace(',', '.'));
    return Number.isFinite(num) ? num : null;
  }

  function calcPublicNutritionKcal(protein, fat, carbs) {
    const p = readPublicNutritionNumber(protein);
    const f = readPublicNutritionNumber(fat);
    const c = readPublicNutritionNumber(carbs);
    if (p == null || f == null || c == null) return null;
    return Math.round((p * 4 + f * 9 + c * 4) * 100) / 100;
  }

  function normalizePublicNutrition(values) {
    const protein = readPublicNutritionNumber(values?.protein);
    const fat = readPublicNutritionNumber(values?.fat);
    const carbs = readPublicNutritionNumber(values?.carbs);
    return {
      protein,
      fat,
      carbs,
      kcal: calcPublicNutritionKcal(protein, fat, carbs),
    };
  }
  ...
  async function loadPublicNutritionTree(tenantId, rootProductId) {
    ...
    (Array.isArray(conversionRows) ? conversionRows : []).forEach((row) => {
      const factor = readPublicNutritionNumber(row.factor);
    ...
    if (productIds.length) {
      ...
      (Array.isArray(links) ? links : []).forEach((link) => {
        unitLinks.set(`${Number(link.product_id)}:${Number(link.unit_id)}:${Number(link.base_unit_id)}`, readPublicNutritionNumber(link.factor));
      });
    }
  ...
  function resolvePublicIngredientQtyInGrams(ingredientRow, tree) {
    ...
    const quantity = readPublicNutritionNumber(ingredientRow?.quantity);
    ...
    const directGramFactor = getPublicNutritionUnitFactor(fromUnitId, gramUnitId, tree);
    ...
    if (fromUnitId === baseUnitId) {
      ...
      if (productFactor != null) qtyInBase = quantity * productFactor;
        ...
        const genericFactor = getPublicNutritionUnitFactor(fromUnitId, baseUnitId, tree);
    ...
    const baseToGramFactor = productBaseToGramFactor != null ? productBaseToGramFactor : getPublicNutritionUnitFactor(baseUnitId, gramUnitId, tree);
    return baseToGramFactor == null ? null : qtyInBase * baseToGramFactor;
  }

  function resolvePublicProductBaseQtyInGrams(product, tree) {
    const baseQty = readPublicNutritionNumber(product?.base_qty);
    ...
    const factor = productFactor != null ? productFactor : getPublicNutritionUnitFactor(baseUnitId, gramUnitId, tree);
    return factor == null ? null : baseQty * factor;
  }

  function buildPublicNutritionResult(productId, tree, stack = []) {
    const id = Number(productId || 0);
    const product = tree.products.get(id);
    if (!product) return { values: normalizePublicNutrition({}), portion: null, incomplete: true };
    if (stack.includes(id)) return { values: normalizePublicNutrition({}), portion: null, incomplete: true };
    const ingredients = tree.ingredientsByProductId.get(id) || [];
    if (!ingredients.length) {
      const values = normalizePublicNutrition({
    ...
    for (const ing of ingredients) {
      const qtyInGrams = resolvePublicIngredientQtyInGrams(ing, tree);
      if (qtyInGrams == null) {
        incomplete = true;
        continue;
      }
      totalGrams += qtyInGrams;
      const child = buildPublicNutritionResult(Number(ing.ingredient_id), tree, [...stack, id]);
    ...
    const portion = normalizePublicNutrition({ protein, fat, carbs });
    const baseQtyGrams = resolvePublicProductBaseQtyInGrams(product, tree);
  ...
  async function applyPublicProductNutrition(rows, tenantId) {
    ...
    for (const row of list) {
      ...
      if (row.blocks_config && row.blocks_config.nutrition === false) {
        row.nutrition_kcal_100g = null;
        row.nutrition_per_100g = null;
        row.nutrition_per_portion = null;
        row.nutrition_incomplete = false;
  ...
  async function loadPublicUnitConversionsPayload(req) {
    const tenantId = helpers.getTenantId(req);
    const [rows] = await db.query(
  ...
  router.get('/unit-conversions', async (req, res) => {
    try {
      return res.json({ ok: true, data: await loadPublicUnitConversionsPayload(req) });
    } catch (e) {
      console.error(e);
      return res.status(500).json({ ok: false, error: 'DB_ERROR' });
    }
  ...
  async function enrichProductsWithDisplayPrice(rows, tenantId, storeId) {
    ...
    for (const r of vaRows) {
      ...
      if (!blocksConfig.variants) continue;
    ...
    for (const row of optionAssignmentRows) {
      ...
      const blocksConfig = blocksConfigMap.get(pid) || getDefaultProductBlocksConfig();
      if (!blocksConfig.options) continue;
    ...
    const ingredientLinesByProductId = new Map();
    {
      ...
      (Array.isArray(ingredientRows) ? ingredientRows : []).forEach((ing) => {
        ...
        const blocksConfig = blocksConfigMap.get(pid) || getDefaultProductBlocksConfig();
        if (!blocksConfig.ingredients) return;
    ...
    for (const row of rows) {
      ...
      const blocksConfig = blocksConfigMap.get(pid) || getDefaultProductBlocksConfig();
      row.blocks_config = blocksConfig;
      const variant = blocksConfig.variants ? variantByProductId.get(pid) : null;
      let displayPrice = await computeDisplayPriceForProduct(row, variant, getConversionFactor, roundPrice);
      const optionAssignments = blocksConfig.options ? (optionAssignmentsByProductId.get(pid) || []) : [];
      const catalogDefaultLines = blocksConfig.ingredients ? [...(ingredientLinesByProductId.get(pid) || [])] : [];
  ...
  async function enrichProductsWithDiscounts(rows, tenantId, storeId) {
    ...
    const [discountRows] = await db.query(
      `SELECT d.*, dp.product_id, dp.category_id, dp.combo_id
       ...
       LEFT JOIN mkt_discount_products dp ON dp.discount_id = d.id AND dp.tenant_id = d.tenant_id
       WHERE d.tenant_id = ? AND d.store_id = ? AND d.is_active = 1 AND d.is_deleted = 0
         AND (
           d.mechanic_type = 'buy_x_get_y'
           OR dp.product_id IN (${placeholders})
           OR dp.category_id IS NOT NULL
         )`,
    ...
    const buyXGetYBadges = [];
    const seenBuyXGetYBadgeIds = new Set();

    for (const disc of discountRows) {
      const mechanicType = normalizePublicMechanicType(disc?.mechanic_type);
      if (mechanicType === 'buy_x_get_y') {
        const discountId = Number(disc.id || 0);
        if (seenBuyXGetYBadgeIds.has(discountId)) continue;
        seenBuyXGetYBadgeIds.add(discountId);
        const mechanic = parsePublicDiscountObject(disc?.mechanic_config_json, {});
        const targetSets = buildPublicProductTargetSets(mechanic?.qualifying_items || []);
        const badgeText = formatPublicBuyXGetYBadge(mechanic);
        if (
          badgeText
          && (
            targetSets.anyProductIds.size
            || targetSets.productIds.size
            || targetSets.categoryIds.size
          )
        ) {
          buyXGetYBadges.push({
            id: discountId,
            priority: Number(disc.priority || 0),
            badge_text: badgeText,
            title: publicDiscountText(disc?.title) || badgeText,
    ...
    buyXGetYBadges.sort((a, b) => (b.priority || 0) - (a.priority || 0));

    for (const row of rows) {
      const pid = Number(row.id);
      const blocksConfig = blocksConfigMap.get(pid) || getDefaultProductBlocksConfig();
      row.blocks_config = blocksConfig;
      row.buy_x_get_y_badge = null;
      for (const promoBadge of buyXGetYBadges) {
        const productItem = { type: 'product', product_id: pid };
        if (matchDiscountTargetScope(promoBadge.target_sets, productItem, catByProduct, 'any')) {
          row.buy_x_get_y_badge = {
            id: promoBadge.id,
            badge_text: promoBadge.badge_text,
            title: promoBadge.title,
  ...
  async function fetchOrderPayload(tenantId, storeId, id, opts = {}) {
    ...
    await enrichPublicOrderItemsWithBuyXGetYBadges(tenantId, storeId, items);
    let discountsJson = [];
    try {
  ...
  async function enrichPublicOrderItemsWithBuyXGetYBadges(tenantId, storeId, items) {
    ...
    const badges = (Array.isArray(discountRows) ? discountRows : [])
      ...
      .map((row) => {
        const mechanic = parsePublicDiscountObject(row?.mechanic_config_json, {});
        const targetSets = buildPublicProductTargetSets(mechanic?.qualifying_items || []);
        if (!targetSets.productIds.size && !targetSets.anyProductIds.size && !targetSets.categoryIds.size) return null;
        const badgeText = formatPublicBuyXGetYBadge(mechanic);
        return {
          id: Number(row?.id || 0),
          priority: Number(row?.priority || 0),
          badge_text: badgeText,
          title: publicDiscountText(row?.title) || badgeText,
          buy_qty: Math.max(1, Number(mechanic?.buy_qty || 0) || 1),
          reward_qty: Math.max(1, Number(mechanic?.reward_qty || 0) || 1),
          repeat_mode: publicDiscountText(mechanic?.repeat_mode).toLowerCase() === 'repeat' ? 'repeat' : 'single',
    ...
    source.forEach((item) => {
      ...
      const productItem = { type: 'product', product_id: productId };
      const badge = badges.find((row) => matchDiscountTargetScope(row.target_sets, productItem, productCategoriesMap, 'any'));
      if (!badge) return;
...
window.location.replace(${JSON.stringify(redirectUrl)});
  ...
  router.get('/categories', async (req, res) => {
    try {
      const result = await loadPublicCategoriesPayload(req);
      res.set('x-public-cache', result.cacheHit ? 'HIT' : 'MISS');
  ...
  router.get('/shop/bootstrap', async (req, res) => {
    try {
      const [categoriesResult, orderConfig, unitConversions] = await Promise.all([
        loadPublicCategoriesPayload(req),
        loadPublicOrderConfigPayload(req),
        loadPublicUnitConversionsPayload(req),
      ]);

      res.json({
        ok: true,
        data: {
          categories: Array.isArray(categoriesResult.payload?.data) ? categoriesResult.payload.data : [],
          orderConfig,
          unitConversions,
        },
      });
      ...
      const { payload, cacheState } = await loadPublicCachedPayload(
        ...
        async () => {
          ...
          for (const sb of setBlocks) {
            const [productsRaw] = await db.query(
              `SELECT bp.product_id, bp.sort_order, bp.is_default, p.name AS product_name, p.description_short AS product_description_short,
                      p.price, p.base_qty, p.base_unit_id, p.unit_id, p.photos_json AS product_photos_json
               FROM prod_combo_block_products bp
               JOIN prod_products p ON p.id = bp.product_id AND p.tenant_id = bp.tenant_id
               LEFT JOIN prod_product_stocks s ON s.tenant_id=p.tenant_id AND s.store_id=? AND s.product_id=p.id
               WHERE bp.tenant_id=? AND bp.block_id=? AND p.is_active=1 AND p.site_visibility=1
            ...
            const products = availableProductsRaw.map((r) => {
              ...
              return {
                ...
                price: Number(r.price) || 0,
                base_qty: Number(r.base_qty || 1) || 1,
                base_unit_id: r.base_unit_id ? Number(r.base_unit_id) : null,
          ...
          const comboProductIds = Array.from(new Set(
            blocks
              .flatMap((block) => Array.isArray(block.products) ? block.products : [])
              .map((product) => Number(product.product_id || 0))
          ...
          if (comboProductIds.length) {
            const getConversionFactor = getConversionFactorMap(tenantId, db);
            const placeholders = comboProductIds.map(() => '?').join(',');
            ...
            const [variantRows] = await db.query(
              `SELECT
               ...
               WHERE va.tenant_id=?
                 ...
                 AND va.is_active=1 AND vg.is_active=1
               ORDER BY va.product_id ASC, va.sort_order ASC, vg.sort_order ASC`,
              [tenantId, ...comboProductIds]
            );

            const variantGroupIds = Array.from(new Set(
              variantRows.map((row) => Number(row.id)).filter((id) => Number.isFinite(id) && id > 0)
            ));
            const variantValueExclusionsByScope = await loadVariantValueExclusionsMap(tenantId, comboProductIds, variantGroupIds);
            ...
            if (variantGroupIds.length) {
              ...
              for (const tier of tiers) {
                const groupId = Number(tier.variant_group_id);
                if (!tiersByGroupId.has(groupId)) tiersByGroupId.set(groupId, []);
                tiersByGroupId.get(groupId).push(tier);
              }
            }
            ...
            for (const row of variantRows) {
              ...
              const groupDefaultIdx = row.group_default_value_index != null ? Number(row.group_default_value_index) : null;
              const assignmentDefaultIdx = row.assignment_default_value_index != null ? Number(row.assignment_default_value_index) : null;
              const groupId = Number(row.id);
              const variantData = buildVisibleVariantData(
                safeJsonArray(row.values),
                tiersByGroupId.get(groupId) || [],
                assignmentDefaultIdx != null ? assignmentDefaultIdx : groupDefaultIdx,
                getVariantValueExclusionSet(variantValueExclusionsByScope, productId, groupId)
              );
              if (!variantData) continue;
            ...
            const [ingredientRows] = await db.query(
              `SELECT
               ...
               ORDER BY i.product_id ASC, i.sort_order ASC, i.id ASC`,
              [pcsUnitId || 0, tenantId, ...comboProductIds]
            );

            await attachPublicNutritionToIngredientRows(ingredientRows, tenantId);
            const ingredientsByProductId = new Map();
            for (const row of ingredientRows) {
              row.ingredient_photos = safeJsonArray(row.ingredient_photos);
              const productId = Number(row.product_id);
              if (!ingredientsByProductId.has(productId)) ingredientsByProductId.set(productId, []);
              ingredientsByProductId.get(productId).push(row);
            }

            const productBlockRows = await loadPublicProductBlockRows(tenantId, comboProductIds);
            ...
            const comboDiscountedPrice = (price) => {
              const p = Number(price) || 0;
              const d = Number(discountPercent) || 0;
              return roundPrice(d >= 100 ? 0 : p * (1 - d / 100));
            };
            const getQtyInBase = async (ingredient, qty) => {
              const baseUnitId = Number(ingredient.ingredient_base_unit_id || ingredient.ingredient_unit_id || 0);
              const fromUnitId = Number(ingredient.unit_id || 0);
              if (!baseUnitId || !fromUnitId) return null;
              if (baseUnitId === fromUnitId) return Number(qty || 0);
              const factor = await getConversionFactor(fromUnitId, baseUnitId);
              return factor != null ? Number(qty || 0) * Number(factor || 0) : null;
            };
            const buildComboProductPreview = async (product) => {
              const productId = Number(product.product_id || 0);
              const blocksConfig = blocksConfigMap.get(productId) || getDefaultProductBlocksConfig();
              const variants = blocksConfig.variants ? (variantsByProductId.get(productId) || []) : [];
              const ingredients = blocksConfig.ingredients ? (ingredientsByProductId.get(productId) || []) : [];
              ...
              let vIdx = vGroup?.default_value_index != null ? Number(vGroup.default_value_index) : 0;
              if (!Number.isFinite(vIdx)) vIdx = 0;
              vIdx = values.length ? Math.max(0, Math.min(vIdx, values.length - 1)) : 0;
              const variantLabel = values[vIdx] != null ? String(values[vIdx]) : '';

              let unitPrice = Number(product.price || 0);
              if (vGroup) {
                unitPrice = await computeDisplayPriceForProduct(
                  {
                    price: product.price,
              ...
              for (const ingredient of ingredients) {
                ...
                const pricePerUnit = ingredient.price_override != null && Number(ingredient.price_override) >= 0
                  ...
                  : catalogBasePrice;
                const currentQtyInBase = await getQtyInBase(ingredient, quantity);
                const baseQtyInBase = await getQtyInBase(ingredient, baseQty);
                const diff = (currentQtyInBase != null && baseQtyInBase != null && Number.isFinite(pricePerUnit))
                  ...
                  : (Number.isFinite(pricePerUnit) ? (quantity - baseQty) * pricePerUnit : 0);
                unitPrice += diff;
              }
              ...
              const ingredientsDisplay = ingredients.map((ingredient) => ({
                ...
                qty: Number(ingredient.quantity ?? 0),
                unit_id: ingredient.unit_id != null ? Number(ingredient.unit_id) : null,
                unit: ingredient.unit_short_title || ingredient.unit_title || ingredient.unit_code || 'г',
              }));
              const hasVariantChoices = values.length > 1;
              const hasAdjustableIngredients = ingredients.some((ingredient) => {
                const minQty = Number(ingredient.quantity_min ?? 0);
              ...
              return {
                ...
                variant_label: variantLabel,
                variant_group_id: vGroup && (vGroup.id || vGroup.variant_group_id) != null ? Number(vGroup.id || vGroup.variant_group_id) : null,
                variant_value_index: vGroup ? vIdx : null,
                variant_group_title: vGroup && (vGroup.title || vGroup.title_label) ? String(vGroup.title || vGroup.title_label) : '',
                variant_unit: vGroup && (vGroup.unit_short_title || vGroup.unit_title || vGroup.unit_code) ? String(vGroup.unit_short_title || vGroup.unit_title || vGroup.unit_code) : '',
                unit_id: vGroup && vGroup.unit_id != null ? Number(vGroup.unit_id) : null,
                ingredients_display: ingredientsDisplay,
                hasConfigurable: Boolean(hasVariantChoices || hasAdjustableIngredients),
                unit_price_before_discount: roundPrice(unitPrice),
                unit_price_override: comboDiscountedPrice(unitPrice),
                variants,
                ingredients,
              };
            };
            ...
            await Promise.all(blocks.map(async (block) => {
              ...
              await Promise.all(products.map(async (product) => {
                product.preview = await buildComboProductPreview(product);
              }));
            }));
          }
  ...
  const COMBOS_CACHE_TTL_MS = 5 * 60 * 1000;
  ...
  router.get('/products/:id', async (req, res) => {
    try {
      ...
      await applyPublicProductNutrition([p], tenantId);
    ...
    try {
      ...
      const { payload, cacheState } = await loadPublicCachedPayload(
        ...
        async () => {
          ...
          const blocksConfigMap = await resolveProductBlocksConfigMap(tenantId, 0, productBlockRows);
  ...
  router.post('/products/batch/details', async (req, res) => {
    try {
      const tenantId = helpers.getTenantId(req);
      const storeId = helpers.getStoreId(req);
      const ids = Array.isArray(req.body?.ids)
        ...
        : [];
      if (!ids.length) return res.json({ ok: true, data: { assignments: {}, ingredients: {}, variants: {}, option_groups: {} } });
      if (ids.length > 200) return res.status(400).json({ ok: false, error: 'TOO_MANY' });

      const sortedIds = ids.slice().sort((a, b) => a - b);
      const cacheKey = makePublicCacheKey('products-batch-details', { tenantId, storeId, ids: sortedIds });
      const { payload, cacheState } = await loadPublicCachedPayload(
        cacheKey,
        ...
        async () => {
          ...
          const assignments = {};
          sortedIds.forEach((pid) => { assignments[pid] = []; });
          ...
          const rootGroupIds = Array.from(new Set(variantRows.map((r) => Number(r.id)).filter((n) => Number.isFinite(n) && n > 0)));
          const rootVariantValueExclusionsByScope = await loadVariantValueExclusionsMap(tenantId, sortedIds, rootGroupIds);
          const tiersByGroupId = new Map();
          if (rootGroupIds.length) {
            const tierPlaceholders = rootGroupIds.map(() => '?').join(',');
            const [tiers] = await db.query(
              `SELECT variant_group_id, min_quantity, discount_percent, sort_order
               FROM prod_variant_discount_tiers
               WHERE tenant_id=? AND variant_group_id IN (${tierPlaceholders})
          ...
          const variants = {};
          sortedIds.forEach((pid) => { variants[pid] = []; });
          for (const v of variantRows) {
            const pid = Number(v.product_id);
            ...
            const variantData = buildVisibleVariantData(
              ...
              tiersByGroupId.get(groupId) || [],
              assignmentDefaultIdx != null ? assignmentDefaultIdx : groupDefaultIdx,
              getVariantValueExclusionSet(rootVariantValueExclusionsByScope, pid, groupId)
            );
            if (!variantData) continue;
            variants[pid].push({
              id: groupId,
          ...
          return { ok: true, data: { assignments, ingredients, variants, option_groups: optionGroups } };
        }
      );
  ...
  router.get('/order-config', async (req, res) => {
    try {
      res.json({ ok: true, data: await loadPublicOrderConfigPayload(req) });
    } catch (e) {
      console.error(e);
      res.status(500).json({ ok: false, error: 'DB_ERROR' });
    }
