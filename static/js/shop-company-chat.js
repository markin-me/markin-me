(function () {
  ...
  const footerIsland = document.getElementById("shopCompanyChatFooterIsland");
  const composer = document.getElementById("shopCompanyChatComposer");
  const sendBtn = composer ? composer.querySelector(".shop-company-chat-composer__btn--send") : null;
  ...
  function hasFeedMessagesForEdgeSpring() {
    return !!(
      thread
      && thread.querySelector
      && thread.querySelector(".shop-company-chat-row[data-message-id]")
    );
  }
  ...
  function setFeedEdgeSpringTouchState(active, edge) {
    const nextActive = active === true;
    const nextEdge = nextActive ? String(edge || "") : "";
    if (!feedEdgeSpringTouchState) {
      if (!nextActive) return;
      feedEdgeSpringTouchState = {
        id: null,
        startX: 0,
        startY: 0,
      startedAtTop: false,
    ...
    feedEdgeSpringTouchState.active = nextActive;
    feedEdgeSpringTouchState.edge = nextEdge;
  }

  function isFeedEdgeSpringTouchActive(edge) {
  ...
  function stopFeedEdgeSpringAnimation() {
    if (!feedEdgeSpringRaf) return;
    cancelAnimationFrame(feedEdgeSpringRaf);
    feedEdgeSpringRaf = 0;
  }

  function getFeedEdgeSpringMaxShift() {
    return CHAT_FEED_EDGE_SPRING_MOBILE_MAX_SHIFT_PX;
  }

  function setFeedEdgeSpringShift(valuePx) {
    const maxShift = getFeedEdgeSpringMaxShift();
    const raw = Number(valuePx || 0);
    const next = Number.isFinite(raw)
      ? Math.max(-maxShift, Math.min(maxShift, raw))
      : 0;
    if (Math.abs(next - feedEdgeSpringShift) < 0.01) return feedEdgeSpringShift;
    feedEdgeSpringShift = next;
    if (feed) {
      if (Math.abs(next) < 0.1) {
        feed.style.setProperty("--shop-company-chat-edge-spring-shift", "0px");
      } else {
        feed.style.setProperty(
          "--shop-company-chat-edge-spring-shift",
          String(Number(next.toFixed(3))) + "px"
        );
      }
    }
    return feedEdgeSpringShift;
  }

  function computeFeedEdgeSpringShift(distancePx, direction) {
    const rawDistance = Math.max(0, Number(distancePx || 0));
    if (rawDistance <= 0) return 0;
    const maxShift = getFeedEdgeSpringMaxShift();
    const resisted = maxShift * (
      1 - (1 / (((rawDistance * CHAT_FEED_EDGE_SPRING_DISTANCE_GAIN) / maxShift) + 1))
    );
    return direction === "bottom" ? -resisted : resisted;
  }

  function releaseFeedEdgeSpring(options) {
    const opts = options || {};
    ...
    feedEdgeSpringRaf = requestAnimationFrame(function step(now) {
      if (!shouldUseFeedEdgeSpring()) {
        feedEdgeSpringRaf = 0;
        feedEdgeSpringVelocity = 0;
        setFeedEdgeSpringShift(0);
        return;
      ...
      const displacement = Number(feedEdgeSpringShift || 0);
      const acceleration = (
        (-CHAT_FEED_EDGE_SPRING_RELEASE_STIFFNESS * displacement)
        - (CHAT_FEED_EDGE_SPRING_RELEASE_DAMPING * feedEdgeSpringVelocity)
      );
      feedEdgeSpringVelocity += acceleration * dt;
      const nextDisplacement = displacement + (feedEdgeSpringVelocity * dt);
      if (
      ...
      setFeedEdgeSpringShift(nextDisplacement);

      if (Math.abs(feedEdgeSpringShift) < 0.24 && Math.abs(feedEdgeSpringVelocity) < 2.4) {
        feedEdgeSpringRaf = 0;
        feedEdgeSpringVelocity = 0;
        setFeedEdgeSpringShift(0);
        return;
      ...
      feedEdgeSpringRaf = requestAnimationFrame(step);
    });
  }

  function pinFeedToEdge(edge) {
    if (!feed) return;
    if (edge === "top") {
      if (Number(feed.scrollTop || 0) > 0) feed.scrollTop = 0;
      return;
    }
    if (edge === "bottom") {
      const maxTop = getFeedMaxScrollTop();
      if (Math.abs(Number(feed.scrollTop || 0) - maxTop) > 0.5) {
  ...
  function isFeedAtBottom() {
    if (!feed) return false;
    const maxTop = getFeedMaxScrollTop();
    if (maxTop <= CHAT_FEED_EDGE_SPRING_EDGE_EPSILON_PX) return true;
    return (maxTop - Number(feed.scrollTop || 0)) <= CHAT_FEED_EDGE_SPRING_EDGE_EPSILON_PX;
  }

  function resolveFeedEdgeSpringTouch(touchList) {
    if (!feedEdgeSpringTouchState || feedEdgeSpringTouchState.id == null || !touchList) return null;
    for (let index = 0; index < touchList.length; index += 1) {
      const touch = touchList[index];
      if (touch && touch.identifier === feedEdgeSpringTouchState.id) return touch;
    }
    return null;
  }

  function beginFeedEdgeSpringTouch(event) {
    if (!shouldUseFeedEdgeSpring()) {
  ...
  function updateFeedEdgeSpringTouch(event) {
    ...
    let desiredEdge = "";
    const maxTop = getFeedMaxScrollTop();
    if (maxTop <= CHAT_FEED_EDGE_SPRING_EDGE_EPSILON_PX) {
      if (Math.abs(dy) > 0.1) desiredEdge = dy >= 0 ? "top" : "bottom";
    } else if (dy > 0 && (state.startedAtTop === true || isFeedAtTop())) {
      desiredEdge = "top";
    } else if (dy < 0 && (state.startedAtBottom === true || isFeedAtBottom())) {
      desiredEdge = "bottom";
    }

    if (!state.locked) {
      if (absY < CHAT_FEED_EDGE_SPRING_TOUCH_LOCK_PX || absY <= absX) return false;
      if (!desiredEdge) return false;
      state.locked = true;
    ...
    stopFeedEdgeSpringAnimation();
    feedEdgeSpringVelocity = 0;
    setFeedEdgeSpringShift(
      computeFeedEdgeSpringShift(outwardDistance, state.edge)
    );
    return true;
  }
  ...
  function endFeedEdgeSpringTouch(options) {
    ...
    if (state.locked || Math.abs(feedEdgeSpringShift) >= 0.1) {
      releaseFeedEdgeSpring();
      return;
    }
    releaseFeedEdgeSpring({ immediate: true });
  ...
  async function unsubscribeWebPushNotifications() {
    if (webPushSyncTimer) {
    ...
    const subscription = registration && registration.pushManager
      ? await registration.pushManager.getSubscription().catch(function () { return null; })
      : null;
  ...
  function isChatNotificationsEnabled() {
    try {
      ...
      return "Notification" in window && String(Notification.permission || "default") === "granted";
    } catch {
      return false;
    }
  }

  async function setChatNotificationsEnabled(enabled) {
    ...
    if (!isWebPushSupported() || !isWebPushSecureContext()) {
      try { localStorage.setItem(chatNotificationsStorageKey, "0"); } catch {}
      await unsubscribeWebPushNotifications();
      return false;
    }

    const permission = await requestMessageAlertNotificationPermission();
    const allowed = permission === "granted";
    try { localStorage.setItem(chatNotificationsStorageKey, allowed ? "1" : "0"); } catch {}
    if (!allowed) {
      await unsubscribeWebPushNotifications();
      return false;
    }
  ...
  function buildComposerDraftStorageKey(clientId) {
    const id = Number(clientId);
    const safeId = Number.isFinite(id) && id > 0 ? Math.trunc(id) : 0;
    return "shop_company_chat_composer_draft_t" + tenantId + "_c" + String(safeId);
  }
  ...
  function readPersistedComposerDraft(clientId) {
    const key = buildComposerDraftStorageKey(clientId);
    try {
      return normalizeComposerDraftValue(localStorage.getItem(key) || "");
    } catch {
      return "";
    }
  }

  function persistComposerDraft(clientId, rawValue) {
    const id = Number(clientId || getActiveChatClientId() || 0);
    if (!Number.isFinite(id) || id <= 0) return "";
    const key = buildComposerDraftStorageKey(id);
    const nextValue = normalizeComposerDraftValue(
      rawValue !== undefined ? rawValue : (input && typeof input.value === "string" ? input.value : "")
    );
    try {
  ...
  function restorePersistedComposerDraft(clientId) {
    const draft = readPersistedComposerDraft(clientId);
    if (input.value !== draft) input.value = draft;
    syncComposerRichPreview({});
    return draft;
  }

  function clearPersistedComposerDraft(clientId) {
    const id = Number(clientId || getActiveChatClientId() || 0);
    if (!Number.isFinite(id) || id <= 0) return;
    const key = buildComposerDraftStorageKey(id);
  ...
  function transferPersistedComposerDraft(fromClientId, toClientId, rawValue) {
    const fromId = Number(fromClientId || 0);
    const toId = Number(toClientId || 0);
    if (!Number.isFinite(fromId) || fromId <= 0) return;
    if (!Number.isFinite(toId) || toId <= 0 || toId === fromId) return;
    const sourceDraft = normalizeComposerDraftValue(
      rawValue !== undefined ? rawValue : readPersistedComposerDraft(fromId)
    );
    if (!sourceDraft) {
      clearPersistedComposerDraft(fromId);
      return;
    }
    const targetDraft = readPersistedComposerDraft(toId);
    if (!targetDraft) {
      persistComposerDraft(toId, sourceDraft);
    }
    clearPersistedComposerDraft(fromId);
  }
  ...
  function refreshChatClientProfileIfNeeded(options) {
    ...
    const currentDraft = persistComposerDraft(currentProfile.id);
    ...
    restorePersistedComposerDraft(nextProfile.id);
  ...
  function buildAutoLinkHref(value, options) {
    ...
    if (isBareDomainAutoLinkValue(raw)) {
      ...
      const startIndex = Number(opts.startIndex || 0);
  ...
  function shouldRetainComposerFocusOnMobileSend() {
    return isMobileChatViewport()
      && shouldUseNativeMobileEmojiKeyboard()
      && overlay.classList.contains("is-open");
  }
  ...
  function finalizeComposerSendUi(options) {
    const opts = options || {};
    const refocusInput = opts.refocusInput === true;
    input.value = "";
    syncComposerRichPreview({});
    if (refocusInput && !input.disabled) {
  ...
  var companyChatHistoryBound = false;
  var companyChatHistoryRestoring = false;

  function isCompanyChatPageMode() {
    return window.matchMedia && window.matchMedia("(max-width: 768px)").matches;
  }

  function syncCompanyChatPageHeader() {
    const pageMode = isCompanyChatPageMode();
    modalHeader.classList.toggle("shop-page-header", pageMode);
    modalHeader.classList.toggle("shop-company-chat-page-header", pageMode);
    closeBtn.classList.toggle("shop-page-header-btn", pageMode);
    closeBtn.setAttribute("aria-label", pageMode ? "Назад" : "Закрыть чат");
    closeBtn.innerHTML = pageMode
      ? '<i class="fas fa-arrow-left" aria-hidden="true"></i>'
      : '<i class="fas fa-times" aria-hidden="true"></i>';
  }

  function ensureCompanyChatHistoryBinding() {
    ...
    window.addEventListener("popstate", function (event) {
      ...
      if (overlay.classList.contains("is-open") && isCompanyChatPageMode()) {
        closeCompanyChat({ fromHistory: true });
      }
    });
    companyChatHistoryBound = true;
  }
  ...
  function openCompanyChat() {
    ...
    document.body.classList.toggle("shop-company-chat-page-active", isCompanyChatPageMode());
  ...
  function createMessageNode(entry, renderKey) {
    ...
    renderRichMessageText(
      text,
      entry.text || "",
      "shop-company-chat-emoji-glyph shop-company-chat-emoji-glyph--inline",
      "shop-company-chat-auto-link"
    );
  ...
  function renderThread() {
    ...
    if (!hasFeedMessagesForEdgeSpring()) {
      releaseFeedEdgeSpring({ immediate: true });
    }
  ...
  function syncEmojiPickerViewportPosition() {
    ...
    if (emojiPopover.classList.contains("is-mobile-sheet")) {
      body.scrollTo({
        left: Math.max(0, targetSection.offsetLeft || 0),
        behavior: "auto",
      });
      return;
    }
  ...
  function bindEmojiPickerDragScroll(body, isMobileSheet) {
    ...
    const isEnabled = function () {
      return typeof isMobileSheet === "function"
        ? isMobileSheet()
        : !!isMobileSheet;
    };

    const setScrollLeft = function (nextLeft) {
  ...
  function renderEmojiPicker() {
    ...
    bindEmojiPickerDragScroll(body, function () {
      return emojiPopover.classList.contains("is-mobile-sheet");
    });
  ...
  function canUseMessageHeartShortcutTarget(target) {
    ...
    return !(
      ...
      || targetEl.closest(".shop-company-chat-reaction-pill")
      || targetEl.closest("[data-chat-order-card-id]")
      || targetEl.closest("[data-reply-jump-id]")
      || targetEl.closest("[data-quick-label]")
      || targetEl.closest(".shop-company-chat-attachment")
    );
  }
  ...
  function consumeMessageHeartDoubleTap(messageId, point) {
    ...
    const now = Date.now();
    const x = Number(dataPoint.x || 0);
    const y = Number(dataPoint.y || 0);
    ...
    if ((now - prev.at) > HEART_DOUBLE_TAP_MS) return false;
    if (Math.abs(prev.x - x) > HEART_DOUBLE_TAP_MOVE_PX) return false;
    if (Math.abs(prev.y - y) > HEART_DOUBLE_TAP_MOVE_PX) return false;
    clearMessageHeartTap();
    return true;
  }
  ...
  function toggleHeartReactionForMessage(messageId) {
    ...
    hideReactionBar();
    hideEmojiPopover();
    return toggleReaction(id, "\u{2764}\u{FE0F}");
  }
  ...
  function setMobileFooterOverlayHeight(valuePx) {
    const raw = Number(valuePx || 0);
    const next = Number.isFinite(raw) && raw > 0 ? Math.round(raw) : 0;
    const prev = Number(overlay.dataset.mobileFooterOverlayHeight || 0);
    if (prev === next) return;
  ...
  function syncMobileScrollDownPosition() {
    if (!scrollDownBtn) return;
    if (!overlay.classList.contains("is-open") || !isMobileChatViewport()) {
      scrollDownBtn.style.removeProperty("right");
      scrollDownBtn.style.removeProperty("bottom");
      return;
    }

    const root = scrollDownBtn.offsetParent instanceof HTMLElement ? scrollDownBtn.offsetParent : scrollDownBtn.parentElement;
    if (!(root instanceof HTMLElement) || !(sendBtn instanceof HTMLElement)) {
      scrollDownBtn.style.removeProperty("right");
      scrollDownBtn.style.removeProperty("bottom");
      return;
    }
    ...
    const btnWidth = Math.max(
      0,
      Number(scrollDownBtn.offsetWidth || 0),
      Number(btnRect.width || 0)
    );
    if (
      rootRect.width <= 0
      || rootRect.height <= 0
    ...
    const nextRight = Math.max(0, rootRect.right - sendCenterX - (btnWidth / 2));
    const nextBottom = Math.max(18, rootRect.bottom - sendRect.top + 16);
    scrollDownBtn.style.right = String(nextRight) + "px";
    scrollDownBtn.style.bottom = String(nextBottom) + "px";
  }

  function getMobileFooterOverlayAnchor() {
    const selectionModeActive = (
      modalBody
      && modalBody.classList.contains("is-selection-mode")
      && !selectionToolbar.classList.contains("hidden")
    );
    if (selectionModeActive) return selectionToolbar;
    if (footerIsland && !footerIsland.classList.contains("hidden")) return footerIsland;
    return composer;
  }

  function computeMobileFooterOverlayHeight() {
  ...
  function updateScrollDownButton() {
    ...
    const wasHidden = scrollDownBtn.classList.contains("hidden");
    scrollDownBtn.classList.toggle("hidden", !shouldShow);
    if (shouldShow && wasHidden) {
      requestAnimationFrame(syncMobileScrollDownPosition);
    }
  ...
  function sendUserMessage(text, options) {
    ...
    if (editingMessageId) {
      ...
      if (!done) return false;
      cancelEditingMessage();
      clearPersistedComposerDraft(getActiveChatClientId());
      hideContextMenu();
      hideReactionBar();
      hideEmojiPopover();
      return true;
  ...
  renderReplyDraftUi();
  restorePersistedComposerDraft(getActiveChatClientId());
  ...
  thread.addEventListener("click", function (event) {
    ...
    if (autoLink && selectedMessageIds.size === 0) {
      return;
    }
  ...
  thread.addEventListener("contextmenu", function (event) {
    if (event.target.closest && event.target.closest("[data-chat-autolink]") && selectedMessageIds.size === 0) return;
  ...
  thread.addEventListener("touchstart", function (event) {
    ...
    if (event.target.closest && event.target.closest("[data-chat-autolink]") && selectedMessageIds.size === 0) {
      clearOrderCardTouchPan();
      clearTouchGesture();
      return;
    }
  ...
  thread.addEventListener("touchend", function (event) {
    ...
    if (!touchGesture) return;
    const finishedGesture = touchGesture;
    ...
    const canUseHeartDoubleTap = (
      ...
      && Math.abs(finishedGesture.lastX - finishedGesture.startX) <= HEART_DOUBLE_TAP_MOVE_PX
      && Math.abs(finishedGesture.lastY - finishedGesture.startY) <= HEART_DOUBLE_TAP_MOVE_PX
    );
  ...
  window.shopCompanyChatNotifications = {
    ...
    getStatus: function () {
      return {
        ...
        secureContext: isWebPushSecureContext(),
        permission: "Notification" in window ? String(Notification.permission || "default") : "unsupported",
      };
    },
  };
  if (!isChatNotificationsEnabled()) {
