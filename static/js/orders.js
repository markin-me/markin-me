(function () {
  ...
  function hydrateOrdersFromCache(cache) {
    ...
    const rawViewYear = cache?.date?.viewYear;
    const rawViewMonth = cache?.date?.viewMonth;
    const viewYear = Number(rawViewYear);
    const viewMonth = Number(rawViewMonth);
    if (
      rawViewYear != null
      && rawViewMonth != null
  ...
  function handleOrderEvent(order, { localOnly = false, skipStageRefresh = false } = {}) {
    ...
    const nextOrder = localOnly ? { ...order } : overlayCourierShadowOrder(order);
    if (wasExisting) {
...
<body>
  ...
  function ensureDateStateInitialized() {
    ...
    const normalizedViewYear = Number(state.date.viewYear);
    const normalizedViewMonth = Number(state.date.viewMonth);
    const hasValidViewDate = Number.isInteger(normalizedViewYear)
      && Number.isInteger(normalizedViewMonth)
      && normalizedViewYear >= 2000
      && normalizedViewMonth >= 0
      && normalizedViewMonth <= 11;
    if (!hasValidViewDate) {
